...
export class AdminConsumoComponent implements OnInit {
  ...
  catchId(i: any){this.Id = i; console.log(this.Id)}
  ...
  {
    this.CartaoConsumoService.obterResById(resId).then(res => {this.restaurante = res;})
    return this.restaurante.nome
  }

  getTxtInfo(_numero: any, _desc: any, _validade: any, _cpf: any, _desconto: any, _nome: any, _restaurante: any) {
    this.consumoModel = { numero: _numero.value, descricao: _desc.value, valor: 0, validade: _validade.value, Cpf: _cpf.value, Desconto: _desconto.value, Nome: _nome.value, restauranteId: _restaurante.value, saldoAtual: 0 };
    if(_numero.value !== ""  && _desc !== "" && _validade !== ""){
    this.CartaoConsumoService.insertConsu(this.consumoModel).then(() => location.reload())
  ...
  async ngOnInit() {
    ...
    if (this.CartaoConsumoService.isAdmin !=="1" && this.CartaoConsumoService.isAdmin !=="3"){ location.href = '/login';}
    await this.CartaoConsumoService.obterConsuByRes().then(consum => { this.consumo = consum;});
    this.consumo.results.validade.forEach(this.converterShortDate(this.consumo.results.validade));
  ...
  async obterConsulById(id: any) {
    if (id != null && id != "") {
      ...
      this.CartaoConsumoService.obterConsuByRes().then(consum => { this.consumo = consum; console.log(consum) });
    }
  }
  ...
  async deleteById(id: any)
  {
    ...
    await this.CartaoConsumoService.deleteConsu(id).then(() => location.reload());
  }
  ...
  async obterConsulByNrList(consumId: any) {
    ...
    this.CartaoConsumoService.obterConsuByNr(consumId).then(consum => { this.consumo = consum; console.log(consum);
      if(this.consumo.results.length >=1 && consumId !== "" && consumId !== null)
      {
        this.CartaoConsumoService.obterConsuByNr(this.consumId).then(consum => { this.consumo = consum; console.log(consum) });
      }
      else
      {
        this.CartaoConsumoService.obterConsuByRes().then(consum => { this.consumo = consum; console.log(consum) });
  ...
  async obterConsulByNrOrName(consumId: any) {
    ...
    this.CartaoConsumoService.obterConsuByNrOrName(consumId, consumId).then(consum => { this.consumo = consum; console.log(consum);
      if(this.consumo.results.length >=1 && consumId !== "" && consumId !== null)
      {
        this.CartaoConsumoService.obterConsuByNrOrName(this.consumId, this.consumId).then(consum => { this.consumo = consum; console.log(consum) });
      }
      else
      {
        this.CartaoConsumoService.obterConsuByRes().then(consum => { this.consumo = consum; console.log(consum) });
  ...
  async getTxtInfoMov(_saldo: any, _tipoMov: any) {
    await this.CartaoConsumoService.obterConsuById(this.Id).then(consum => { this.consumo = consum;});
    this.mov = {cartaoConsumoId: this.Id, saldo: _saldo, tipoMov: _tipoMov};
    if(this.mov.saldo !== "" && this.mov.tipoMov !== "")
    {
      this.dataMovAtual= new Date;
        this.valorMovAtual = this.mov.saldo;
  ...
  async getTxtInfoMovWithNro(_saldo: any, _tipoMov: any) {
    await this.CartaoConsumoService.obterConsuByNr(this.consumId).then(consum => { this.consumo = consum;});
     if(this.consumo !== null && this.consumId !== "" && this.consumId !== null && _saldo !== "" && _saldo !== null)
    {
      this.index=0;
      this.mov = {cartaoConsumoId: this.consumo.cartaoConsumoId, saldo: _saldo, tipoMov: _tipoMov};
        this.dataMovAtual= new Date;
        this.valorMovAtual = this.mov.saldo;
        let soma = (parseFloat(this.consumo.saldoAtual)+parseFloat(_saldo))
        this.consumoModel = {cartaoConsumoId: this.consumo.cartaoConsumoId, numero: this.consumo.numero, descricao: this.consumo.descricao, valor: this.consumo.valor, validade: this.consumo.validade, Cpf: this.consumo.cpf, Desconto: this.consumo.desconto, Nome: this.consumo.nome, restauranteId: this.consumo.restauranteId, saldoAtual: soma };
        this.soma = soma;
        this.CartaoConsumoService.insertMov(this.mov)
      
    
    }
    else{window.alert("Cartão consumo não encontrado!"); this.reloading()}
